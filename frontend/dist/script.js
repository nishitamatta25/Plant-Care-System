...
async function identifyPlant(imageData) {
    try {
        ...
        if (!PLANT_API_KEY || PLANT_API_KEY === 'YOUR_API_KEY') {
            throw new Error('Invalid Pl@ntNet API key. Please set a valid API key.');
        }

        const apiUrl = `${PLANT_API_URL}?api-key=${PLANT_API_KEY}`;
        console.log('Using API URL with key:', apiUrl);

        // Call the Pl@ntNet API directly (CORS should be enabled in your Pl@ntNet account)
        console.log('Sending request to Pl@ntNet API:', apiUrl);
